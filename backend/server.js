...
// GET /api/employees (Paginated employee list with optional search query)
app.get('/api/employees', async (req, res) => {
  try {
    ...
    const result = await getEmployees({ page, limit, search });
    res.json(result);
  } catch (err) {
...
function validateEmployeeData({ name, age, email, dept, salary, id, newId }) {
  const errors = [];
...
app.post('/api/employees', async (req, res) => {
  try {
    const { id, name, age, email, dept, salary } = req.body;
    
    const errors = validateEmployeeData({ name, age, email, dept, salary, id });
    if (errors.length > 0) {
...
app.put('/api/employees/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { newId, name, age, email, dept, salary } = req.body;

    const errors = validateEmployeeData({ name, age, email, dept, salary, newId });
    if (errors.length > 0) {
      return res.status(400).json({ error: 'Validation failed', details: errors });
    }
